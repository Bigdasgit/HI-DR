...
class VITA(nn.Module):
    ...
    def __init__(self, voc_size, ehr_adj, ddi_adj, ddi_mask_H, gumbel_tau, att_tau, emb_dim=64, device=torch.device('cpu')):
    ...
    def make_query(self, input_disease_embdding, medications):
        ...
        input1 = self.MLP_layer3(input_disease_embdding).squeeze(-1) 
        input2 = self.MLP_layer2(input1) 
        current = input2[-1:, :]
        ...
        visit_diag_embedding = self.mlp_layer(picked.transpose(-2,-1)).view(batch_size, max_visit_num, emb_dim) 
    ...
    def encode(self, diseases, procedures, medications, d_mask_matrix, p_mask_matrix, m_mask_matrix, seq_length, dec_disease, stay_disease, dec_disease_mask, stay_disease_mask, dec_proc, stay_proc, dec_proc_mask, stay_proc_mask, max_len=20):
        ...
        # print(gumbel_pick_index)
 
        input_disease_embdding = torch.cat([input_disease_embdding[:1, :, :], pre_queries]) # (seq, 71, emb_dim)
        # print("input_disease_embdding: ", input_disease_embdding.shape)
        
        ## save query
        query = input_disease_embdding[-1:, :, :]
        query = query.transpose(1, 2)
        query = self.MLP_layer2(query).squeeze(2)
        ...
        return input_disease_embdding, encoded_medication, cross_visit_scores, last_seq_medication, last_m_mask, drug_memory, counts, gumbel_pick_index, query
    ...
    def forward(self, diseases, procedures, medications, d_mask_matrix, p_mask_matrix, m_mask_matrix, seq_length, dec_disease, stay_disease, dec_disease_mask, stay_disease_mask, dec_proc, stay_proc, dec_proc_mask, stay_proc_mask, epoch_saved_patient_embedding):
        device = self.device
        batch_size, max_seq_length, max_med_num = medications.size()
        max_diag_num = diseases.size()[2]
        max_proc_num = procedures.size()[2]
        
        input_disease_embdding, encoded_medication, cross_visit_scores, last_seq_medication, last_m_mask, drug_memory, count, gumbel_pick_index, query = self.encode(diseases, procedures, medications, d_mask_matrix, p_mask_matrix, m_mask_matrix, 
        ...
        print("epoch_saved_patient_embedding: ", len(epoch_saved_patient_embedding))
        ...
        return output_logits, count, gumbel_pick_index, cross_visit_scores_numpy, epoch_saved_patient_embedding
