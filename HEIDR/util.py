...
def sequence_output_process(output_logits, filter_token):
    """Generate the final correct sequence. Output_logits represent the probability at each position, and filter_token represents SOS and END."""
    pind = np.argsort(output_logits, axis=-1)[:, ::-1]  # Sort in descending order at each position according to probability.
    
    out_list = []   # The generated results.
    break_flag = False
    for i in range(len(pind)):
        # Iterate through all values on pind sequentially.
        # The break_flag is used to determine whether to exit the loop for sentence generation.
        if break_flag:
            break
        ...
        for j in range(pind.shape[1]):
            ...
            if label in filter_token:
                break_flag = True
                break
            # If encountering a medication that hasn't appeared before, continue generating.
            # Otherwise, continue to the next medication with a higher probability.
    ...
    # Sort all medications in `out_list` according to their probabilities.
...
def ddi_rate_score_mimic_iv(record, path='data/mimic-iv/ddi_A_final2.pkl'):
    # ddi rate
    ddi_A = dill.load(open(path, 'rb'))
    all_cnt = 0
    dd_cnt = 0
    ...
    if training:
        ...
        for i in range(batch_size): 
            for j in range(seq_length[i]): 
                ...
                if testing:
                    logits_flatten.append(logits[j])  # beam search
                else:
                    logits_flatten.append(logits[i,j,:max_len,:].detach().cpu().numpy())     
